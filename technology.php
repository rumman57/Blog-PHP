...
<div class ="cl">
 <?php
 include("connect.php");
  $music="select * from posts ";
 $run_music=mysqli_query($conn,$music);
 while($row1=mysqli_fetch_array($run_music))
 { 
      if($row1['category']=="Technology")
	  $music_category = $row1['category'];
 }
 
   $per_page_post=4;
   $data4=mysqli_num_rows(mysqli_query($conn,"select * from posts where category='$music_category'"));
   $page_needed=ceil($data4/$per_page_post);
   if (isset($_GET["page"]))
	   { 
           $page  = $_GET["page"];
       } 
 ...
 $sel="select * from posts where category='$music_category' ORDER BY 1 DESC LIMIT $start,$per_page_post";
